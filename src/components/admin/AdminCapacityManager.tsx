    ...
    return (
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 glass-effect p-8 rounded-[2.5rem] border border-white/5 space-y-6">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h3 className="text-xl font-bold text-white">Capacity Configuration</h3>
                                <p className="text-xs text-gray-500 uppercase tracking-widest font-black">System Rules</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
                        <div className="space-y-4">
                            <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 ml-2">Max Simultaneous Projects</label>
                            <div className="flex items-center gap-4">
                                <input
                                    type="number"
                                    className="input-field !py-3 w-24 text-center text-lg font-bold"
                                    value={config?.maxActiveProjects || 2}
                                    onChange={(e) => handleUpdateConfig({ maxActiveProjects: parseInt(e.target.value) })}
                                    min="1"
                                />
                                <p className="text-sm text-gray-400">Determines when status switches to &apos;Limited&apos; or &apos;Booked&apos;.</p>
                            </div>
                        </div>

                        <div className="space-y-4">
                            <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 ml-2">Manual Override</label>
                            <div className="flex gap-2">
                                {["none", "available", "limited", "booked"].map((mode) => (
                                    <button
                                        key={mode}
                                        onClick={() => handleUpdateConfig({ manualOverride: mode === "none" ? null : mode })}
                                        className={`px-3 py-2 rounded-xl text-[9px] font-black uppercase tracking-tighter border transition-all ${(config?.manualOverride === mode || (mode === "none" && !config?.manualOverride))
            ...
            <div className="glass-effect rounded-[2.5rem] border border-white/5 overflow-hidden">
                <div className="p-8 border-b border-white/5 flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-sm text-gray-500 mt-1">Manage current workload and expected timelines.</p>
                    </div>
                    <button
