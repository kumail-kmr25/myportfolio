            ...
            <div className="grid grid-cols-1 gap-6">
                {messages.map((msg) => (
                    <div key={msg.id} className={`group relative glass-effect rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-8 border transition-all ${msg.replied ? 'border-white/5 bg-white/[0.02] opacity-70' : 'border-blue-500/20 bg-blue-500/[0.03] shadow-lg shadow-blue-500/5'}`}>
                        ...
                        {/* Actions */}
                        <div className="absolute top-6 right-6 sm:top-8 sm:right-8 flex gap-2 sm:gap-3 z-10">
                            <button
                                onClick={() => onToggleReplied(msg.id, msg.replied)}
                                className={`p-2 sm:p-3 rounded-xl transition-all shadow-xl cursor-pointer ${msg.replied
                            ...
                            <button
                                ...
                                className="p-2 sm:p-3 bg-red-500/10 text-red-500 rounded-xl hover:bg-red-500 hover:text-white transition-all shadow-xl border border-red-500/20 cursor-pointer"
                            >
                                <Trash2 size={16} />
                            </button>
                        </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 sm:gap-12">
                            ...
                            <div className="space-y-6">
                                <div className="flex items-center gap-4">
                                    <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white text-lg font-bold shadow-lg shadow-blue-500/20">
                                        {msg.name[0]}
                                    </div>
                                    <div className="min-w-0 pr-24 sm:pr-0">
                                        <h3 className="text-base sm:text-lg font-bold text-white truncate">{msg.name}</h3>
                                        <p className="text-gray-500 text-xs sm:text-sm truncate">{msg.email}</p>
                                    </div>
                                </div>
                            ...
                            {/* Message Content */}
                            <div className="lg:col-span-2 space-y-6 bg-white/[0.03] p-6 sm:p-8 rounded-[1.5rem] sm:rounded-[2rem] border border-white/5 relative">
                                ...
                                <p className="text-gray-300 leading-relaxed text-xs sm:text-sm whitespace-pre-wrap">
                                    {msg.message}
                                </p>

                                <div className="pt-6 sm:pt-8 mt-4 border-t border-white/5 flex flex-col sm:flex-row sm:items-center gap-4">
                                    <a
                                        ...
                                        className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-white text-black rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-500 hover:text-white transition-all shadow-xl"
                                    >
                                        <ExternalLink size={14} />
                                        Send Reply
                                    </a>
                                    {msg.timeline && (
                                        <div className="sm:ml-auto flex items-center gap-2 text-[9px] text-gray-500 font-bold uppercase tracking-widest">
