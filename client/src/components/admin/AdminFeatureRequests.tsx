...
export default function AdminFeatureRequests({ requests, onUpdate }: { requests: FeatureRequest[], onUpdate: () => void }) {
    ...
    return (
        <div className="space-y-8">
            <div className="bg-white/5 p-6 sm:p-8 rounded-[1.5rem] sm:rounded-[2rem] border border-white/5">
                <h2 className="text-lg sm:text-xl font-bold text-white">Community Feature Requests</h2>
                <p className="text-xs sm:text-sm text-gray-500 mt-1">Track and manage suggestions from your visitors.</p>
            </div>
                ...
                {requests.map((request) => {
                    ...
                    return (
                        <motion.div
                            ...
                            className="p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] bg-white/[0.02] border border-white/5 hover:border-white/10 transition-all group flex flex-col md:flex-row md:items-center justify-between gap-6 sm:gap-8"
                        >
