            ...
            <div className="w-full max-w-md glass-effect rounded-[2.5rem] p-12 border border-white/10 shadow-2xl">
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    <div className="space-y-1">
                        ...
                        <input type="text" className={`${getFieldClasses("name", formData.name)} transition-all duration-300`} placeholder="Your full name" value={formData.name} onChange={(e) => handleFieldChange("name", e.target.value)} required />
                        {getMatchLabel("name", formData.name)}
                    </div>
                    <div className="space-y-1">
                        <label className="text-sm font-medium text-gray-300">Email</label>
                        <input type="email" className={`${getFieldClasses("email", formData.email)} transition-all duration-300`} placeholder="your@email.com" value={formData.email} onChange={(e) => handleFieldChange("email", e.target.value)} required />
                        {getMatchLabel("email", formData.email)}
                    </div>
                    <div className="space-y-1">
                        <label className="text-sm font-medium text-gray-300">Phone Number</label>
                        <input type="tel" className={`${getFieldClasses("phone", formData.phone)} transition-all duration-300`} placeholder="10-digit phone number" value={formData.phone} onChange={(e) => handleFieldChange("phone", e.target.value)} required pattern="[0-9]{10}" maxLength={10} />
                    ...
                    <button type="submit" disabled={isLoading} className="btn-primary w-full disabled:opacity-50">{isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : "Register"}</button>
                </form>
                <p className="text-center text-gray-500 text-sm mt-6">
                    Already registered? <Link href="/admin" className="text-white hover:text-blue-400 transition-colors">Login here</Link>
                </p>
            </div>
        </div>
    );
