        ...
        <div className="space-y-8">
            ...
            {subTab === "patterns" ? (
                <div className="space-y-6">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white/5 p-6 sm:p-8 rounded-[1.5rem] sm:rounded-[2rem] border border-white/5 gap-6">
                        <div>
                            <h2 className="text-lg sm:text-xl font-bold text-white">Diagnostic Patterns</h2>
                            <p className="text-xs sm:text-sm text-gray-500 mt-1">Manage rules for the smart diagnostic tool.</p>
                        </div>
                        <button
            ...
            {(editingPattern || isAdding) && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/90 backdrop-blur-sm">
                    <div className="bg-[#080808] border border-white/10 rounded-[2.5rem] sm:rounded-[3rem] p-6 sm:p-10 max-w-2xl w-full shadow-2xl relative overflow-y-auto max-h-[90vh]">
                        <button
                            onClick={() => { setEditingPattern(null); setIsAdding(false); }}
                            className="absolute top-6 right-6 sm:top-8 sm:right-8 text-gray-500 hover:text-white"
                        >
                            <X size={24} />
                        </button>

                        <h2 className="text-xl sm:text-2xl font-black text-white uppercase tracking-tighter mb-6 sm:mb-8">
