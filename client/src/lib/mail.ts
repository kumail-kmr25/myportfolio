...
export async function sendContactNotification({
    ...
    try {
        const { data, error } = await resend.emails.send({
            from: "Kumail KMR <onboarding@resend.dev>", // Transition to custom domain in production
        ...
        <div style="font-family: sans-serif; padding: 20px; color: #333; line-height: 1.6;">
          ...
          <p style="margin-top: 30px; font-size: 12px; color: #888; border-top: 1px solid #eee; pt: 10px;">
            Submitted via Kumail KMR's Portfolio at ${new Date().toLocaleString()}
          </p>
        </div>
      `,
        });
    ...
    try {
        ...
        <div style="font-family: sans-serif; padding: 20px; color: #333; line-height: 1.6;">
          ...
          <p style="margin-top: 30px; font-size: 12px; color: #888; border-top: 1px solid #eee; padding-top: 10px;">
            Submitted via Kumail KMR's Admin System at ${new Date().toLocaleString()}
          </p>
        </div>
      `,
        });
...
export async function sendHireNotification({
    ...
    try {
        const { data, error } = await resend.emails.send({
            from: "Kumail KMR Hire <onboarding@resend.dev>",
            to: process.env.CONTACT_EMAIL || "ka6307464@gmail.com",
        ...
        <div style="font-family: sans-serif; padding: 20px; color: #333; line-height: 1.6;">
          ...
          <p style="margin-top: 30px; font-size: 12px; color: #888; border-top: 1px solid #eee; padding-top: 10px;">
            Submitted via Kumail KMR's "Hire Me" Flow at ${new Date().toLocaleString()}
          </p>
        </div>
      `,
        });
